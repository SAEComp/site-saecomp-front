...
import { Product, AdminOrder, ProductStatistics } from '../../types';
...
const StatsManagement: React.FC = () => {
    ...
    const recentOrdersColumns: ITableColumn<AdminOrder>[] = [
        ...
        },
        {
            ...
            render: (_, order) => (
                <div className="text-sm text-gray-900">
                    {formatDate(order.createdAt ? (typeof order.createdAt === 'string' ? order.createdAt : order.createdAt.toISOString()) : (order.updatedAt ? (typeof order.updatedAt === 'string' ? order.updatedAt : order.updatedAt.toISOString()) : new Date().toISOString()))}
                </div>
            )
        }
    ];
