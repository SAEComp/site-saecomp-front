...
const FeedbackEdition = () => {
  ...
  const [detailedAnswerIndex, setDetailedAnswerIndex] =
  ...
  const searchDetailedAnswer = async (id = -1) => {
    ...
    setDetailedAnswerIndex(answers.evaluationId);
  ...
  useEffect(() => {
    ...
    searchDetailedAnswer(detailedAnswerIndex !== null ? detailedAnswerIndex : -1);

  }, [detailedAnswerIndex]);

  const handleReviewSelect = (id: number) => {
    console.log(detailedAnswer)
    console.log(detailedAnswerIndex)
    setDetailedAnswerIndex(prevId => (prevId === id ? null :id))
  }
    ...
    <div className="flex flex-col bg-green-500 w-full">
      ...
      <div className="flex flex-row  mx-[5%] my-[20px] gap-[5%] justify-around">
        <div className=" h-[1104px] w-[300px] bg-green-700 rounded-lg overflow-hidden">
          <ReviewList
            detailedAnswerIndex={detailedAnswerIndex}
        ...
        <div className="  w-[130px] sm:w-[260px] md:w-[390px] lg:w-[60%] xl:w-[70%] 2xl:w-[80%] ">
          <CardEdition detailedAnswer={detailedAnswer} />
        </div>
      </div>
    </div>
  );
